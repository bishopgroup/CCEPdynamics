function [Ul,GradUl] = pFarFieldReal_old(Parameter,Particle)
%{
...
dx3U = bsxfun(@minus,xi(3,:)', 2*Parameter.domainLength(3) - xj(3,:)); % (nParticle x nParticle)
RijU = sqrt(dx1.^2 + dx2.^2 + dx3U.^2);              % (nParticle x nParticle)

% Compute Green's functions and reflections off the walls
[G, dG, dGor, d2G] = Green(rij, Parameter.splittingAlpha);      % (nParticle x nParticle)
[GL, dGL, dGLor, d2GL] = Green(RijL, Parameter.splittingAlpha); % (nParticle x nParticle)
[GU, dGU, dGUor, d2GU] = Green(RijU, Parameter.splittingAlpha); % (nParticle x nParticle)
