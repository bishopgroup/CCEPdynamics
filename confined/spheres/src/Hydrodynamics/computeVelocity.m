...
% Particle.farFieldDipole = ones(Particle.nParticle,3,3);
...
% %    Particle.farFieldDipole(nP,:,:) = [1 2 3; 4 5 6; 7 8 9];
% %end
% 
% [globalVelocity, globalVelocityGradient, globalVelocityLaplacian] = ...
%     mFarFieldWave(Parameter, Particle, Mesh);
...
%Particle.farFieldDipole = reshape(x(3*Particle.nParticle+1:end),...
%    Particle.nParticle, 3,3);


[lhs,velocity] = leftHandSide(x, Parameter, Particle, Mesh, nearFieldResistance);
Particle.velocity = velocity;
%Particle.dipole = Particle.farFieldDipole;
%tmp = reshape(Particle.farFieldDipole(1,:,:),3,3)
...
%  Dij = Sij + Aij
%
Particle.farFieldDipole = zeros(Np, 3, 3);

Particle.farFieldDipole(:,1,1) = Particle.farFieldStresslet(:,1);
Particle.farFieldDipole(:,1,2) = Particle.farFieldStresslet(:,2) ...
    + 0.5 * Particle.farFieldTorque(:,3);
Particle.farFieldDipole(:,1,3) = Particle.farFieldStresslet(:,3) ...
    - 0.5 * Particle.farFieldTorque(:,2);

Particle.farFieldDipole(:,2,1) = Particle.farFieldStresslet(:,2) ...
    - 0.5 * Particle.farFieldTorque(:,3);
Particle.farFieldDipole(:,2,2) = Particle.farFieldStresslet(:,4);
Particle.farFieldDipole(:,2,3) = Particle.farFieldStresslet(:,5) ...
    + 0.5 * Particle.farFieldTorque(:,1);

Particle.farFieldDipole(:,3,1) = Particle.farFieldStresslet(:,3) ...
    + 0.5 * Particle.farFieldTorque(:,2);
Particle.farFieldDipole(:,3,2) = Particle.farFieldStresslet(:,5) ...
    - 0.5 * Particle.farFieldTorque(:,1);
Particle.farFieldDipole(:,3,3) = -Particle.farFieldDipole(:,1,1) ...
    - Particle.farFieldDipole(:,2,2); % stresslet is traceless
